...
class UserUseCaseTest {
    ...
    @Mock
    private com.uni.pe.storyhub.domain.repository.LikeByUserRepository likeByUserRepository;
    @Mock
    ...
    void getPublicProfile_Success() {
        ...
        when(blogRepository.findByAuthorUsernameAndPublicadoTrueAndRemovedFalse(anyString(), any()))
                .thenReturn(blogPage);
    ...
    @Test
    void getPublicProfile_NoBlogs_Success() {
        when(userRepository.findByUsername(anyString())).thenReturn(Optional.of(user));
        when(blogRepository.findByAuthorUsernameAndPublicadoTrueAndRemovedFalse(anyString(), any()))
                .thenReturn(org.springframework.data.domain.Page.empty());
        when(toastIdGenerator.nextId()).thenReturn(1);

        ApiResponse<com.uni.pe.storyhub.application.dto.response.PublicUserProfileResponse> response = userService
                .getPublicProfile("testuser", org.springframework.data.domain.PageRequest.of(0, 10));

        assertNotNull(response);
        assertTrue(response.getData().getBlogs().isEmpty());
    }
}
